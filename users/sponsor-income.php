...
if( isset($_POST['userN']) ) {
       ...
       $ref_id =  $row['myid']; 
       $query= mysqli_query($dbc,"SELECT * FROM income_history WHERE receiver_id='$ref_id' AND `remark`='Referral bonus'"); 
       $row = mysqli_fetch_array($query);
    }
		if(isset($_GET['page'])){
		...
		$query=mysqli_query($dbc,"SELECT * FROM income_history WHERE receiver_id='$myid' AND `remark`='Referral bonus'");
    ...
    $receiver_name = get_fullname($myid);
	$ref_id = '1509990793-740778577';
    $ref_name = get_fullname($ref_id);
	$Amount = $Amount+4;
			   	$history_query = "INSERT INTO income_history(ref_name, ref_id, receiver_name, receiver_id,Amount, Remark, Status ) 
       VALUES('$ref_name','$ref_id','$receiver_name','$myid','$Amount','Referral bonus', 'Paid')";
                ...
                <div class="panel-body">
                  ...
                  <table class="table datatable dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info">
                    ...
                    <tbody>
                          	          <?php
                            $me= $userRow['email'];
                            $query=mysqli_query($dbc,"SELECT * FROM income_history WHERE receiver_id='$myid' AND `remark`='Referral bonus' LIMIT ".(($page-1)*$itemsperpage).','.$itemsperpage);
                            $i= 1;
